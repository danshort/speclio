...
When no background color is configured for a view, the system SHALL render exactly as it does today: all areas not covered by styled segments display the terminal emulator's default background. No post-processing or wrapping SHALL occur.
...
The system SHALL support a per-view background color configuration via a Theme struct on the Model. The background color SHALL be a `lipgloss.TerminalColor` value. Setting it to `nil` or `lipgloss.NoColor` SHALL disable the background (terminal default fallback).
