...
- **Distribution — DECIDED: no Apple Developer Program yet.** No paid account until the App Store move or sustained growth justifies the $99/yr + notarization ops. Consequences, and the plan around them: (a) **Apple Silicon requires at least an ad-hoc signature to run at all**, so the build still does free **ad-hoc signing** (`codesign --sign -`) — this is not Developer-ID and does not need an account; (b) without notarization, **Gatekeeper blocks first launch** ("Apple cannot check it for malicious software"), so the user must right-click → Open once (or `xattr -d com.apple.quarantine`), documented in the README and cask caveats; (c) **Homebrew cask** is the distribution channel (it applies the quarantine attribute by default — a `--no-quarantine` install is the smoother path for those who opt in). Developer-ID signing + notarization (tasks 6.1/6.2) are **deferred**, not removed; they slot in when the account exists.
